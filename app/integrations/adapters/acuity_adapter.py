...
class AcuityAdapter(BaseAdapter):
            ...
            "fields": [
                {
                    ...
                    "type": "text",
                    "required": True,
                    "help_text": "Ihre Acuity Scheduling User ID.",
                },
                {
                    "key": "api_key",
                    "label": "API Key",
                    "type": "password",
                    "required": True,
                    "help_text": "Ihr Acuity Scheduling API Key.",
                },
            ],
        }
    ...
    async def get_contacts(
        ...
        return SyncResult(
            ...
            records_fetched=0,
            contacts=[],
            metadata={"note": "Acuity Scheduling does not support contact sync."},
        )
    ...
    async def test_connection(self, config: dict) -> "ConnectionTestResult":
        ...
        return ConnectionTestResult(
            success=True,
            message="Acuity Scheduling-Adapter geladen (Verbindungstest nicht implementiert).",
        )
