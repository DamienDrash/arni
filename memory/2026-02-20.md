...
## Wichtiger Kontext
- Das aktive `arni`-Worktree ist aktuell sauber (keine unstaged/staged Änderungen im Repo selbst).
...
- Frontend basePath/Auth-Fixes abgeschlossen (`/arni/api/auth/*` sicher).
...
- `.env` auf Postgres umgestellt (`DATABASE_URL=postgresql+psycopg://...@postgres:5432/arni`).
- `arni-core`/`arni-telegram` neu gestartet und live geprüft (auth + admin endpoints ok).

## Restore Fortsetzung (Knowledge + Telegram)
- Editor-Lesbarkeit für Knowledge/Member-Memory/System-Prompt final gefixt (`TiptapEditor` + `.arni-editor` CSS in `globals.css`).
- `arni-telegram` Health dauerhaft korrigiert: eigener Worker-Healthcheck in `docker-compose.yml` (Python `/proc/1/cmdline`), Status jetzt `healthy`.
- Verifiziert:
...
- Verifikation:
  ...
  - Frontend neu gestartet/build erfolgreich, Seiten erreichbar (`/arni`, `/arni/knowledge`, `/arni/member-memory`, `/arni/system-prompt` => 200).
...
- Verifikation:
  ...
  - Deployment `arni-core` durchgeführt.
...
- Verifikation live:
  ...
  - Externe Seiten wieder erreichbar: `/arni`, `/arni/member-memory`, `/arni/settings` => 200.
...
- Verifikation:
  ...
  - `/arni/live` wieder `200` nach Frontend-Rebuild.
...
- Commit `e36d27dc`:
  ...
  - `/arni/api/send` in `frontend/app/api/send/route.ts` auf optionalen, token-gated Legacy-Proxy umgestellt (`NEXT_ENABLE_LEGACY_SEND_PROXY`, `LEGACY_SEND_TOKEN`, `WHATSAPP_BRIDGE_SEND_URL`).
...
- Deploy durchgeführt via `docker compose up -d --force-recreate arni-core arni-telegram arni-frontend`.
- Post-deploy verifiziert:
  - `GET http://127.0.0.1:8000/health` => `status=ok`
  - `https://services.frigew.ski/arni/settings/integrations` => 200
  - `https://services.frigew.ski/arni/settings/automation` => 200
...
- Deploy & Verify:
  - `docker compose up -d --force-recreate arni-core arni-frontend`
  - `GET /health` => ok
  - `https://services.frigew.ski/arni/settings/general` => 200
...
- Verifikation:
  ...
  - `npm run build` => ok
- Deploy + Live-Checks:
  - `https://services.frigew.ski/arni/knowledge` => 200
  - `https://services.frigew.ski/arni/member-memory` => 200
...
- Verifikation:
  ...
  - Deploy + Live-Checks:
    - `/arni/login` => 200
    - `/arni/knowledge` => 200
    - `/arni/member-memory` => 200

## Incident Analyse: Direktes Logout nach Login
- Root Cause bestätigt:
  - `POST /arni/api/auth/login` liefert zwar `access_token`, setzt aber aktuell keinen `Set-Cookie` Header.
...
- Technischer Fix (Commit `f277b9a4`):
  - `frontend/lib/auth.ts`: `arni_access_token` wieder in `sessionStorage` als Fallback gespeichert.
...
- Live-Checks:
  - `/arni/login` => 200
  - `/arni/api/auth/me` mit Bearer-Token => 200
...
- Verifikation:
  - `GET /arni/api/admin/knowledge/status` mit Bearer => 200
  - Live-Seiten: `/arni/knowledge`, `/arni/member-memory`, `/arni/login` => jeweils 200
...
- 1) Auth Cookie-only finalisiert:
  ...
  - Live-Smoke: `POST /arni/api/auth/login` setzt `arni_access_token` + `arni_csrf_token`; `GET /arni/api/auth/me` nur via Cookie => 200.
...
- Verifikation:
  ...
  - Deploy: `docker compose up -d --force-recreate arni-core arni-frontend`.
  - Live: `/arni/live`, `/arni/audit`, `/arni/login` jeweils 200.
...
- Frontend umgesetzt:
  ...
  - Session-Sync verbessert via `arni:session-updated` Event in `frontend/lib/auth.ts`.
