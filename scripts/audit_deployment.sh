#!/bin/bash
echo "🔍 ARNI Deployment Report" > deployment_report.txt
...
echo -e "\n3. Container Logs (Last 50 lines - arni_core):" >> deployment_report.txt
docker logs --tail 50 arni_core 2>&1 >> deployment_report.txt

echo -e "\n4. Redis Logs (Last 20 lines):" >> deployment_report.txt
docker logs --tail 20 arni_redis 2>&1 >> deployment_report.txt
