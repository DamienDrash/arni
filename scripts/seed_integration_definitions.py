#!/usr/bin/env python3
"""Seed script: Populate integration_definitions from CONNECTOR_REGISTRY.

This script reads the CONNECTOR_REGISTRY and all registered adapters,
then upserts the data into the integration_definitions table.  It can
be run repeatedly (idempotent) and is safe for both development and
production environments.

Usage:
    # From the project root:
    python scripts/seed_integration_definitions.py

    # Or via Docker:
    docker exec ariia-core python scripts/seed_integration_definitions.py
...
# ─── Seed Data ──────────────────────────────────────────────────────────────
# This is the canonical mapping from CONNECTOR_REGISTRY + internal adapters
# to integration_definitions rows.  Each entry corresponds to one row.

INTEGRATION_DEFINITIONS: list[dict] = [
    # ── Messaging ───────────────────────────────────────────────────────
    {
        "id": "whatsapp",
        "name": "WhatsApp",
...
def seed(database_url: str | None = None) -> None:
    """Upsert all integration definitions into the database."""
    ...
    print(f"✓ Successfully upserted {inserted} integration definitions.")

    # Print summary
    with Session(engine) as session:
        result = session.execute(text(
            "SELECT category, COUNT(*) FROM integration_definitions GROUP BY category ORDER BY category"
        ))
        print("\nSummary by category:")
        for row in result:
            print(f"  {row[0]:15s} → {row[1]} integrations")

        total = session.execute(text("SELECT COUNT(*) FROM integration_definitions")).scalar()
        print(f"\n  {'TOTAL':15s} → {total} integrations")
