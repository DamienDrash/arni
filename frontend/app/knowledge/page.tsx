...
export default function KnowledgePage() {
  ...
  const loadFiles = useCallback(async () => {
    ...
    if (!res.ok) {
      setError(`${t("knowledge.loading")} Error (${res.status}).`);
      setFiles([]);
  ...
  const save = useCallback(async (opts?: { force?: boolean; silent?: boolean }) => {
    ...
    setStatus(opts?.silent ? t("knowledge.autosaving") : t("knowledge.saving"));
    setError("");
    ...
    if (!res.ok) {
      if (res.status === 409) {
        setConflict(true);
        setStatus(t("knowledge.conflict"));
        setError(t("knowledge.conflictError"));
        return;
      }
      setStatus("");
      setError(`${t("common.error")} (${res.status}).`);
      return;
    }
    ...
    setDirty(false);
    setConflict(false);
    setStatus(opts?.silent ? t("knowledge.autosaved") : t("knowledge.saved"));
  ...
  async function reindexNow() {
    setReindexing(true);
    setStatus(t("knowledge.reindexing"));
    setError("");
    try {
      const res = await apiFetch(`/admin/knowledge/reindex${slugParam}`, { method: "POST" });
      if (!res.ok) {
        const body = await res.json().catch(() => ({}));
        setError(body?.detail || `${t("common.error")} (${res.status}).`);
        setStatus("");
        return;
      }
      setStatus(t("knowledge.saved"));
      await Promise.all([loadFiles(), loadMeta()]);
    } finally {
      setReindexing(false);
    }
  ...
  function createDraftFile() {
    ...
    setDirty(true);
    setConflict(false);
    setStatus(t("knowledge.draftCreated"));
    setNewName("");
  }
    ...
    <div className="grid grid-cols-1 gap-4 min-h-[calc(100svh-12rem)]">
      ...
      {isSystemAdmin && (
        <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", background: T.surface, border: `1px solid ${T.border}`, borderRadius: 10 }}>
          <span style={{ fontSize: 12, color: T.textDim, whiteSpace: "nowrap" }}>{t("knowledge.viewTenant")}</span>
          <select
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: 10 }}>
        <Card style={{ padding: 12 }}><div style={{ fontSize: 11, color: T.textDim }}>{t("knowledge.documents")}</div><div style={{ fontSize: 21, fontWeight: 800, color: T.text }}>{meta?.files_count ?? files.length}</div></Card>
        <Card style={{ padding: 12 }}><div style={{ fontSize: 11, color: T.textDim }}>{t("knowledge.indexChunks")}</div><div style={{ fontSize: 21, fontWeight: 800, color: T.accent }}>{meta?.vector_count ?? 0}</div></Card>
        <Card style={{ padding: 12 }}>
          <div style={{ fontSize: 11, color: T.textDim }}>{t("knowledge.lastIngest")}</div>
          <div style={{ fontSize: 13, fontWeight: 700, color: ingestColor }}>{meta?.last_ingest_status || t("common.never")}</div>
          <div style={{ marginTop: 2, fontSize: 11, color: T.textMuted }}>{meta?.last_ingest_at ? new Date(meta.last_ingest_at).toLocaleString("de-DE") : t("common.never")}</div>
        </Card>
        <Card style={{ padding: 12, display: "flex", alignItems: "center", justifyContent: "flex-end" }}>
          ...
          >
            <RefreshCw size={13} /> {reindexing ? t("knowledge.reindexing") : t("knowledge.reindex")}
          </button>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[270px_1fr] gap-4 min-h-[calc(100svh-18rem)]">
        <Card style={{ padding: 14, overflow: "auto" }}>
          ...
          <div style={{ display: "grid", gap: 4 }}>
            {files.length === 0 && <div style={{ fontSize: 12, color: T.textDim }}>{t("knowledge.noDocuments")}</div>}
            {files.map((f) => (
              <button
                key={f}
        ...
        <Card style={{ padding: 0, display: "flex", flexDirection: "column" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 14px", borderBottom: `1px solid ${T.border}` }}>
            <div style={{ color: T.textDim, fontFamily: "monospace", fontSize: 12 }}>{selectedFile || t("knowledge.selectFile")}</div>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <span style={{ color: conflict ? T.warning : dirty ? T.text : T.textDim, fontSize: 12 }}>
                {conflict ? t("knowledge.conflict") : dirty ? t("common.loading") : status}
              </span>
              {conflict && (
                <>
                  <button
          ...
          <div style={{ padding: 14, flex: 1 }}>
            {error ? (
              ...
              <div style={{ color: T.textDim, fontSize: 13 }}>{t("knowledge.selectFile")}</div>
            )}
            {meta?.last_ingest_error && (
              <div style={{ marginTop: 10, fontSize: 12, color: T.danger }}>{t("knowledge.ingestError", { error: meta.last_ingest_error })}</div>
            )}
          </div>
        </Card>
      </div>
