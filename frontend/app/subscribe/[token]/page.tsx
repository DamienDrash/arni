...
/* ── Inline tokens (standalone page, no NavShell) ─────────────────────── */
const C = {
  bg: "#0a0a0a",
  card: "#1a1a1a",
  border: "#2a2a2a",
...
export default function SubscribePage() {
  ...
  useEffect(() => {
    ...
    const url = base
      ? `${base}/public/subscribe/${token}`
      : `/proxy/public/subscribe/${token}`;
    ...
    return (
      <div style={{ minHeight: "100vh", background: C.bg, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
        <div style={{ textAlign: "center", maxWidth: 420 }}>
          <div style={{
            width: 72, height: 72, borderRadius: "50%", background: "rgba(0,214,143,0.12)",
          ...
          <p style={{ fontSize: 14, color: C.textMuted, lineHeight: 1.6 }}>
            Vielen Dank, {firstName}! Sie erhalten in Kürze eine Bestätigung
            {info?.tenant_name ? ` von ${info.tenant_name}` : ""}.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
      ...
      <div style={{
        ...
        borderRadius: 20, border: `1px solid ${C.border}`, padding: 32,
      }}>
        {/* Header */}
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          ...
          <p style={{ fontSize: 13, color: C.textMuted, margin: 0 }}>
            {info?.tenant_name || "Studio"}
          </p>
          {info?.description && (
            <p style={{ fontSize: 14, color: C.textDim, marginTop: 12, lineHeight: 1.5 }}>
              {info.description}
            </p>
          )}
        </div>
